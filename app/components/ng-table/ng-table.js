    ...
    app.factory('ngTableParams', ['$q', '$log', function ($q, $log) {
        ...
        var ngTableParams = function (baseParameters, baseSettings) {
            ...
            this.url = function (asString) {
                ...
                /**
                 * Alteração feita para transformar os nomes dos parâmetros
                 * para o que o nosso serviço usa.
                 */
                ...
                for (var key in params) {
                    if (params.hasOwnProperty(key)) {
                        var item = params[key],
                            name = encodeURIComponent(translateKey(key));
                        if (typeof item === "object") {
                            for (var subkey in item) {
                                if (!angular.isUndefined(item[subkey]) && item[subkey] !== "") {
                                    ...
                                    if (asString) {
                                        if (pname == "filter") {
                                            pairs.push(pname + "=" + encodeURIComponent(subkey) + "=*" + encodeURIComponent(item[subkey])) + "*";
                                        } else {
                                            pairs.push(pname + "=" + encodeURIComponent(subkey) + "," + item[subkey]);
                                        }

                                    } else {
                                        if (pname == "filter") {
                                            pairs[pname] = encodeURIComponent(subkey) + "=*" + item[subkey] + "*";
                                        } else {
                                            pairs[pname] = encodeURIComponent(subkey) + "," + item[subkey];
                                        }
                                    }
                            ...
                            /**
                             * Reduz o numero da página em 1, pois nosso serviço usa um
                             * esquema que inicia a contagem de zero
                             */
                            if (name == "page") {
                ...
                compile: function (element) {
                    ...
                    return function (scope, element, attrs) {
                        ...
                        if (!element.hasClass('ng-table')) {
                            scope.templates = {
                                header: (attrs.templateHeader ? attrs.templateHeader : 'components/ng-table/header.html'),
                                pagination: (attrs.templatePagination ? attrs.templatePagination : 'components/ng-table/pager.html')
                            };
